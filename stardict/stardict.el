...
;; My definition of appending to hash

(defun appendhash (key newValue table)
  (let ((presentValue (gethash key table)))
    (puthash key (if presentValue
		     (append presentValue (list newValue))
		   (list newValue)) table)))
      ...
      (let ((rpt (make-progress-reporter "read index: " 0 (1- word-count))))
        (dotimes (i word-count)
          ...
          (let (p word offset size)
            ...
            (appendhash word (cons offset size) idx-ht)
            )))
...
(defun stardict-lookup (dict word)
  ...
  (let ((info (gethash word (nth 1 dict)))
	...
	;(progn
	  ;(get-buffer-create "*Dict*")
	(with-temp-buffer ;"*Dict*"
					;(erase-buffer)
					;(goto-char (point-min))
	  (dolist (singleCons info)
	    (progn
	      (insert-file-contents (nth 2 dict) nil (car singleCons) (+ (car singleCons) (cdr singleCons)))
	      (goto-char (+ (point) (cdr singleCons)))
	      (insert "\n\n")))
	    (buffer-string)
	    ;nil))))))
	    )))))
(defun stardict-lookup-list (directoryList word)
  "This will call `stardict-lookup' in all dicts in the `DIRECTORYLIST' with input word as `WORD' and concatenate the output to a buffer named `*Dict*'"
  (when directoryList
    (set 'buffer (get-buffer-create "*Dict*"))
     (with-current-buffer buffer
;;    (with-output-to-temp-buffer "*Dict*"
      (erase-buffer)
      (stardict-lookup-list-internal directoryList word)
      (goto-char (point-min))
      (local-set-key "n" (lambda () (interactive) (Man-next-section 1)))
      (local-set-key "p" (lambda () (interactive) (Man-previous-section 1))))))



(defun stardict-lookup-list-internal (directoryList word)
  "This looks up `WORD' in all dicts in `DIRECTORYLIST' and appends the results to current buffer"
  (when directoryList
    (set 'result (stardict-lookup (car directoryList) word))
    (when result
      (insert (upcase (gethash "bookname" (nth 0 (car directoryList)))))
      (insert "\n===============================================================================================================\n\n")
      (insert result))
    (stardict-lookup-list-internal (cdr directoryList) word)))
